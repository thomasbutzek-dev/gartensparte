...
export default async function HomeModules({ settings }: { settings: VereinsSettings }) {
  ...
  if (!showBoard && !showWeather && !showVerband && !showNewsletter) return null;
  ...
  const box = `${card} flex h-full flex-col`;
              ...
              {notices.map((item) => (
                <li key={item.id} className="flex gap-3">
                  {item.imageFile ? (
                    ...
                    <img src={`/api/schaukasten/${item.id}`} alt="" className="h-14 w-20 shrink-0 rounded object-cover" />
                  ) : null}
                  <span>
                    <span className="block font-medium">{item.title}</span>
                    {item.body ? <span className="line-clamp-2 block text-sm text-stone-600">{item.body}</span> : null}
                  </span>
                </li>
              ))}
            </ul>
        ...
        <section className={box}>
          ...
          <ul className="space-y-3">
            {verbandItems.slice(0, 3).map((item) => (
              <li key={item.url}>
                <a href={item.url} target="_blank" rel="noreferrer" className="flex gap-3 font-medium text-green-800 hover:underline">
                  {item.image ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={item.image} alt="" referrerPolicy="no-referrer" className="h-16 w-14 shrink-0 rounded bg-stone-100 object-contain" />
                  ) : null}
