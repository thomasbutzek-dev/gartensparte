...
describe("Bilder verkleinern", () => {
  ...
  it("erkennt JPG und PNG auch ohne Dateityp, wie beim Ziehen unter Windows", () => {
    expect(imageTypeFromFile(new File([noisyPng(8, 8)], "hoffmeister.png"))).toBe("image/png");
    expect(imageTypeFromFile(new File([noisyPng(8, 8)], "foto.JPG"))).toBe("image/jpeg");
    expect(imageTypeFromFile(new File([noisyPng(8, 8)], "bild.heic"))).toBeNull();
  });

  it("speichert ein Foto ohne gemeldeten Dateityp", async () => {
    const dir = mkdtempSync(join(tmpdir(), "gartensparte-foto-"));
    const file = new File([noisyPng(80, 80)], "vorstand.png");
    const saved = await saveImageUpload(dir, file);
